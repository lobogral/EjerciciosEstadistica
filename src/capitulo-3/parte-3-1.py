...
escribir("3.21")
f = x**Fracción(1,2)
k = 1/integrar(f, (x,0,1))
escribir("a) k =", k)
escribir("b)")
F = integrar(k*f.subs(x,t), (t,0,x))
...
escribir("")
escribir("3.27")
f = Fracción(1,2000)*exp(-x/2000)
F = integrar(f.subs(x,t), (t,0,x))
escribir("a) F(x) =")
escribir("|", 0, "               , x < 0")
escribir("|", F, ", x >= 0")
...
a = 1
b = oo
escribir("a) P(X > 1) =", integrar(f, (x,a,b)))
F = integrar(f.subs(x,t), (t,1,x))
escribir("b) F(x) =")
escribir("|", 0, "         , x < 1")
escribir("|", F, ", x >= 1")
