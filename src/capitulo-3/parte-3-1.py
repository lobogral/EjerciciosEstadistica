...
escribir("")
escribir("3.31")
fdp = Trozos((Fracción(1,4)*exp(-y/4), (y>=0)), 
             (0, otroCaso))
...
fdp = Trozos(((y**4)*(1-y)**3, (0<=y) & (y<=1)), 
             (0, otroCaso))
k = 1/cont.ProbTotal(fdp,y)
escribir("a) k =", k)
Y = ContinuousRV(y, k*fdp)
prob = redondear(cont.Prob((0 < Y) & (Y < 0.5)).evalf(),4)
escribir("b) P(0 < Y < 0.5) =", prob)
prob = redondear(cont.Prob((0.8 < Y) & (Y < 1)).evalf(),4)
