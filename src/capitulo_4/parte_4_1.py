...
f = E_disc.dist_a_dp(dist, x)
E_disc.establecer_dp(f)
E = redondear(E_disc.E(x).evalf(), 2)
escribir("E(X) = $", E)
...
f = E_disc.dist_a_dp(dist, x)
E_disc.establecer_dp(f)
E = redondear(E_disc.E((2*x+1)**2).evalf(), 0)
...
f = E_disc.dist_a_dp(dist, x)
E_disc.establecer_dp(f)
E = redondear(E_disc.E(1200*x-50*x**2).evalf(), 0)
...
f = Trozos((2*(1-x), (0<x) & (x<1)),
           (0, otro_caso))
E_cont.establecer_fdp(f)
E = redondear(E_cont.E(x**2).evalf()*5000, 2)
...
E_disc.establecer_dp(f)
E = redondear(E_disc.E(x*y**2).evalf(), 1)
escribir("a) g(X,Y) = XY^2, E(g(X,Y)) =", E)
E = redondear(E_disc.E(x).evalf(), 2)
escribir("b) E(X) =", E)
E = redondear(E_disc.E(y).evalf(), 2)
escribir("c) E(Y) =", E)
...
prob = simplify(dist_cont.prob(y>E))
